...
def save_to_csv(player_name, score, correct_presses, incorrect_presses):
    ...
    with open(csv_filepath, mode='a', newline='') as file:
        ...
        writer.writerow([player_name, score, 
                         ' '.join([f"({x:.2f},{y:.2f},{z:.2f})" for (x, y, z) in correct_presses]), 
                         ' '.join([f"({x:.2f},{y:.2f},{z:.2f})" for (x, y, z) in incorrect_presses])])
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                if check_square(grid):
                    score += 1
                    correct_presses.append((float(OFFSET_X + col * (2 * CIRCLE_RADIUS + CIRCLE_MARGIN) + CIRCLE_RADIUS), 
                                            float(OFFSET_Y + row * (2 * CIRCLE_RADIUS + CIRCLE_MARGIN) + CIRCLE_RADIUS), 
                                            0.0))
                else:
                    incorrect_presses.append((float(OFFSET_X + col * (2 * CIRCLE_RADIUS + CIRCLE_MARGIN) + CIRCLE_RADIUS), 
                                              float(OFFSET_Y + row * (2 * CIRCLE_RADIUS + CIRCLE_MARGIN) + CIRCLE_RADIUS), 
                                              0.0))
                    game_over = True
